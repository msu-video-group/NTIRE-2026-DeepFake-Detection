...
class ScientificRichProgressBar(RichProgressBar):
    def get_metrics(self, trainer, pl_module):
        metrics = super().get_metrics(trainer, pl_module)
        for k, v in metrics.items():
            if isinstance(v, float):
                if v != 0 and (abs(v) >= 1e4 or abs(v) < 1e-4):
                    metrics[k] = f"{v:.4e}"
                else:
                    metrics[k] = f"{v:.4f}"
        return metrics
...
def make_dataloaders(shard_dir, val_dataset_dir, batch_size=32, num_workers=16):
    ...
    """
    train_dataset = AIGenDetDataset.read_from_shards(shard_dir, shard_nums=[0, 1, 2, 3, 4], transform=distort_images)
...
class BaselineDetector(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = timm.create_model("vit_medium_patch16_gap_384.sw_in12k_ft_in1k", pretrained=True)
...
class TrainingModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer, T_0=10, T_mult=2, eta_min=1e-6)
...
def main():
    ...
    val_dataset_dir = "/root/users/deepfake_bench/data/pixelprose/NTIRE_val/v3"
    ...
    trainer = pl.Trainer(
        ...
        # callbacks=[ScientificRichProgressBar()]
    )
    
    ckpt_path = os.path.join(checkpoint_dir, "epoch=29-val_loss=0.4451.ckpt")
